...
namespace ITGlobal.MarkDocs.Blog.Impl
{
    internal static class BlogPostMetadataProvider
    {
        ...
        public static bool TryParse(IPage page, ICompilationReportBuilder report, out DateTime date, out string slug)
        {
            ...
            {
                report.Warning(page.RelativePath, $"Page \"{page.Id}\" skipped - wrong path format");
                return false;
            }

            if (!int.TryParse(parts[0], out var year) || year < 0)
            {
                report.Warning(page.RelativePath, $"Skipped - \"{parts[0]}\" is not a valid year");
                return false;
            }
            ...
            if (month <= 0 || month > 12)
            {
                report.Warning(page.RelativePath, $"Skipped - \"{parts[1]}\" is not a valid month");
                return false;
            }
            ...
            report.Warning(page.RelativePath, $"Skipped - \"{parts[2]}\" is not a valid day of {year}/{month}");
            return false;
        }
    }
}